            ...
            @else
                @foreach($transacoes as $transacao)
                    <tr class="hover:bg-gray-300 dark:hover:bg-gray-800">
                        ...
                        <td class="{{ ($transacao->tipo == 'receita')? 'text-green-600': 'text-red-600' }} px-6 py-4 whitespace-nowrap capitalize"><span>{{($transacao->tipo != 'receita')? '-': '+'}}</span> AO {{ number_format($transacao->valor, 2, ',', '.')}}</td>
                    </tr>
                @endforeach
            @endif
        </tbody>
